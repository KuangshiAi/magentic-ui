...
class ParaViewAgentConfig(McpAgentConfig):
    ...
    with Docker container management OR local ParaView installation.

    Inherits all MCP agent configuration plus adds ParaView-specific settings.
    """

    # ParaView mode selection
    use_local_paraview: bool = False  # If True, use local ParaView instead of Docker

    # Local ParaView settings (used when use_local_paraview=True)
    conda_env: str = "paraview_mcp"  # Conda environment with ParaView installed

    # ParaView Docker settings (used when use_local_paraview=False)
    ...
    # Common settings for both Docker and local modes
    pvserver_port: int = 11111
    data_dir: Path | None = None
    auto_gui_connect: bool = True
    gui_connect_wait_time: int = 10  # Seconds to wait for GUI to connect before MCP
...
The ParaView GUI is available through a web browser for visual feedback."""
    )
