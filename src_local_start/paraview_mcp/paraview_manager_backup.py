...
class ParaViewManager:
    ...
    def load_data(self, file_path):
        ...
        try:
            ...
            # IMPORTANT: When MCP server runs on host but ParaView runs in Docker,
            # file paths that are absolute (starting with /) are likely inside the container.
            # We should NOT validate these paths locally, but let ParaView handle them.
            is_likely_container_path = (
                os.path.isabs(file_path) and
                (file_path.startswith('/home/MCPagent/') or
            ...
            # Only check file existence if it's a local path (not a container path)
            if not is_likely_container_path and not os.path.exists(file_path):
            ...
            # For container paths, log that we're trusting the path exists in the container
            if is_likely_container_path:
                self.logger.info(f"Using container path (skipping local validation): {file_path}")
